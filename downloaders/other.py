...
class OtherDownloader(BaseDownloader):
    ...
    async def _download_with_youget(self, video_info: VideoInfo, show_progress: bool = True) -> Path:
        """使用you-get下载视频"""
        output_path = self.output_dir / f"{video_info.platform}_{video_info.video_id}.mp4"
        
        cmd = ["you-get", "-o", str(self.output_dir), "-O", f"{video_info.platform}_{video_info.video_id}", video_info.url]
